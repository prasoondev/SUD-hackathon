...
// Local AI service as fallback when OpenAI is not available
class LocalAIService {
  async generateExerciseRecommendations(userStats: UserStats): Promise<ExerciseRecommendation[]> {
    ...
    if (recommendations.length < 3) {
      recommendations.push({
        ...
        type: "strength",
        points: 55,
        reason: "Regular strength training is essential for overall health and metabolism."
      });
    }
  ...
  async chatWithAI(message: string, userStats: UserStats): Promise<string> {
